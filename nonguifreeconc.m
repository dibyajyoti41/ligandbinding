...
%%%In this non-Gui version we start with the defined inputs such as 
     ...
     % last. This code calculates the free concentration of the ligands.
...
%Calculation of free concentration
    tic
        sets.theoryGen.concN=conc(1);
        sets.theoryGen.concY=conc(2);
        sets.theoryGen.concEtbr=conc(3);
        sets.model.netropsinBindingConstant=netropsinBindingConstant;
        sets.model.yoyoBindingConstant=bindingConstantsVals{2};
        sets.model.EtbrBindingConstant=bindingConstantsVals{3};
        disp('Computing free concentrations');
        ntIntSeq = nt2int( lambdaSequence.Sequence, 'ACGTOnly',1);

        probsBinding = @(x) freeconc(ntIntSeq, x(1),x(2),x(3),sets.model.netropsinBindingConstant,sets.model.yoyoBindingConstant,sets.model.EtbrBindingConstant, 1000, 2);

        x0 = [sets.theoryGen.concN sets.theoryGen.concY sets.theoryGen.concEtbr];

        fun = @(x) x0-x-mean(probsBinding(x))*sets.theoryGen.concDNA*0.25; %stoichiometry 0.25 (4 lattice size)
        ...
        sets.theoryGen.concN = xNew(1);
        sets.theoryGen.concY = xNew(2);
        sets.theoryGen.concEtbr = xNew(3);
        toc
        disp('Finished computing free concentrations');
        conc(1)=sets.theoryGen.concN;
        conc(2)=sets.theoryGen.concY;
        conc(3)=sets.theoryGen.concEtbr;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
i=1;
          numbers=0:ligandLengths(i)-1;  
     for j=nSize-ligandLengths(i):-1:1 %1 = 'A' %2 = 'C', 3 = 'G', 4 = 'T'
